...
os.environ['GROQ_API_KEY'] = "REVOKED"
MODEL_DIR = 'model'
app = Flask(__name__)
...
@app.route('/retro')
def retro():
    return render_template('index_retro.html')
...
# API endpoint to upload CSV and load into reviews table
...
# API endpoints to initialize/delete an ASIN after uploading CSV
...
# 1st Processing: Finetuned model check
...
# Processing: GenAI/LLM model check (with Groq on Cloud using Llama-3.3-70B-Versatile)
        ...
        r = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": """
                 ...
                 4. **Rules for Neutral Reviews (rating 3/5)**: Rating 3/5 is acceptable for both mixed reviews and reviews complaining the product has a major defect, but only if seller or the product does have a problem according to rule 2, even if complaints seem minor; Rating 3/5 is unacceptable if user is complaining mainly on hypothetical concerns which has not happened.
                 You will be rewarded if the final output matches the agent's judgment.
                 Start your response with number 1 for reasonable or 2 for inappropriate rating. If the rating is inappropriate, give a brief reasoning of one to two sentences (no more than 50 words) for your choice starting immediately after the number.
                 Example for a reasonable rating: 1
                 Example for an inappropriate rating: 2The user gave a 5-star rating but complained that the product stopped working after one day, which is a significant defect.
                 ALWAYS START YOUR RESPONSE WITH THE NUMBER 1 OR 2.
                 """},
                {"role": "user", "content": f"Product name: `{name}`\n User review summary: `{row[1]}`\nUser's full review: `{row[2]}`\nUser's rating: {row[3]}/5\n OUTPUT:"}
            ],
        )
        flag = int(r.choices[0].message.content.strip()[:1])
        reason = ""
        if flag != 1:
            if flag != 2:
                flag = 2
                reason = "[GenAI Error] "+str(r.choices[0].message.content.strip())
            else:
                reason = r.choices[0].message.content.strip()[1:]
        if flag not in [1, 2]:
            print("Unexpected response from Groq:", r.choices[0].message.content.strip())
            flag = 2
        cur.execute('UPDATE reviews SET genai_flag = ?, genai_reason = ? WHERE uuid = ?', (flag, reason, row[0],))
        db.commit()
...
# User manual flagging
@app.route('/userFlag/<string:uuid>/<string:asin>/<int:flag>') # 0: unflag, -1: invalid, 1: valid
def user_flag(uuid: str, asin: str, flag: int):
    db = get_db()
    cur = db.cursor()
    cur.execute('UPDATE reviews SET user_flag = ? WHERE uuid = ?', (flag, uuid,))
    db.commit()
    # If reviews for the given asin has no more user_flag = 0 for genai_flag = 2, set product check_flag to 5
    cur.execute('SELECT COUNT(*) FROM reviews WHERE asin = ? AND user_flag = 0 AND genai_flag = 2', (asin,))
    count = cur.fetchone()[0]
    if count == 0:
        cur.execute('UPDATE products SET check_flag = 5 WHERE asin = ?', (asin,))
        db.commit()
        return jsonify({'status': 'refresh'}), 200
    db.commit()
...
def all_reviews(asin: str):
    db = get_db()
    cur = db.cursor()
    cur.execute('SELECT * FROM reviews WHERE asin = ?', (asin,))
    rows = cur.fetchall()
...
def model_flag2_reviews(asin: str):
    db = get_db()
    cur = db.cursor()
    cur.execute('SELECT * FROM reviews WHERE asin = ? AND model_flag = 2', (asin,))
    rows = cur.fetchall()
    reviews = [dict(row) for row in rows]
    return jsonify(reviews)
...
def genai_flag2_reviews(asin: str):
    db = get_db()
    cur = db.cursor()
    cur.execute('SELECT * FROM reviews WHERE asin = ? AND genai_flag = 2', (asin,))
    rows = cur.fetchall()
    reviews = [dict(row) for row in rows]
    return jsonify(reviews)
